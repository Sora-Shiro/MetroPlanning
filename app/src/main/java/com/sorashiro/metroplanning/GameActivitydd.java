...
public class GameActivitydd extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_game_dd);
    ...
    private void moveMetroToPosition(final int beforeX, final int beforeY, final int afterX, final int afterY) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mMapImg[beforeX][beforeY].setText("");
                mMapImg[afterX][afterY].setBlockBase(2, metro);
                mMapImg[afterX][afterY].setText(text);
            }
        });
    ...
    private void updateMetroPassenger(final int x, final int y) {
        ...
        final TextView stationTv = mMapImg[curStationX][curStationY];
        //主线程操作
        mHandler.post(new Runnable() {
            @Override
            public void run() {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (failType == ConstantValue.FAIL_TIME_OUT) {
                    ...
                    mMapImg[bx][by].setText("");
                    mMapImg[x][y].clearBlockBase();
                    mMapImg[x][y].setText("");
                    mMapImg[x][y].setBackgroundDrawable(brust);
        ...
        switch (z) {
            case 0:
                if (type == ConstantValue.TURNOUT) {
                    ...
                    mMapImg[x][y].setText(station.getPassenger() + "\n" + station.getPriority());
                }
                break;
            case 1:
